...
async def test_add_new_get_booking(room_id, date_from, date_to, status_code, authenticated_ac: AsyncClient):
...
async def test_crud_booking(authenticated_ac: AsyncClient):
    create_response = await authenticated_ac.post("/bookings", params={
        "room_id": 1,
    ...
    read_deleted_response = await authenticated_ac.get(f"/bookings/{created_booking_id}")
